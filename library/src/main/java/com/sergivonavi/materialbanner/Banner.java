...
public class Banner extends ViewGroup implements BannerInterface {
    ...
    private void loadDimens(Context context) {
        ...
        mContainerPaddingTopOneLine = getDimen(R.dimen.mb_container_padding_top_one_line);
    ...
    private void retrieveAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (a.hasValue(R.styleable.Banner_buttonTextAppearance)) {
            int textAppearance = a.getResourceId(R.styleable.Banner_buttonTextAppearance,
        ...
        if (a.hasValue(R.styleable.Banner_lineColor)) {
            mLine.setBackgroundColor(a.getColor(R.styleable.Banner_lineColor,
                    ContextCompat.getColor(context, R.color.mb_line_default)));
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecSize - widthUsed - iconViewWidth - buttonsWidth >= messageViewWidth) {
            ...
            onMultiline();
            // TODO: stack buttons
            // Doesn't fit, check if both buttons fit in one line
            if (widthSpecSize - widthUsed >= buttonsWidth) {
                // The both buttons fit in one line, default placement
            } else {
                // The both buttons don't fit in one line, stack them
            }
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        mContentContainer.layout(0, 0, mContentContainer.getMeasuredWidth(),
                mContentContainer.getMeasuredHeight());
